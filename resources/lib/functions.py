...
def main_entry_point():
    ...
    if (len(params) == 1 and request_path
        ...
        clone_default_skin()
        # Around line 140-180 in functions.py, add this elif case:
    elif mode == "SHOW_INFO":
...
def show_info_dialog(params, item=None):
    ...
    People are no longer requested for listings (very slow on large
    libraries), so they are loaded here, lazily, for just this one item.
    """
